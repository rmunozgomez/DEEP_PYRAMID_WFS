...
def main():
    ...
    output_dir = os.path.join("./TEST/CL", test_name)
    ...
    # -----------------------------------------------------
    # MEMORY-SAFE REFERENCE SETUP
    #
    # Read ONLY the first training configuration on CPU to obtain the shared
    # quantities required to generate the common atmosphere. No NN, no WFS and
    # no model-specific basis are loaded here.
    #
    # Therefore the first real load_model_bundle() happens only inside the
    # closed-loop model loop below: load one model -> run -> release -> next.
    # -----------------------------------------------------
    first_model_info = MODELS_TO_TEST[0]
    first_train_path = first_model_info["train_path"]
    first_stage = first_model_info["stage"]
    first_config_path = os.path.join(first_train_path, "config", "all_cfg.pt")
    ...
    ref_stage_cfg = ref_cfg["stages_cfg"][0 if first_stage is None else first_stage]
    ...
    # generate_atmosphere_sequence() only needs cfg to resolve the source
    # wavelength. This remains entirely on CPU and contains no NN/WFS objects.
    ref_bundle_for_atmosphere = {
        "cfg": ref_cfg,
    }

    # Filled lazily when each model is actually executed. This avoids any
    # preliminary model/basis pass while preserving the same summary structure.
    basis_by_model_info = {}

    if selected_noise_level == "all":
        noise_levels_to_run = list(range(0, 11))
    else:
        noise_levels_to_run = [validate_noise_level(selected_noise_level)]

    # -----------------------------------------------------
    # Generate the atmosphere only once with the new Torch ATMOSPHERE backend.
    # No OOPAO and no dataset generator are used. The exact same temporal
    # realization is reused by every model/noise level, preserving the original
    # common-atmosphere comparison.
    # -----------------------------------------------------
    atmosphere_sequence = generate_atmosphere_sequence(
        ref_bundle=ref_bundle_for_atmosphere,
        resolution=ref_resolution,
        model_telescope_diameter=ref_diameter,
    ...
    global_summary = {
        "test_name": test_name,
        "device": device,
        "selected_noise_level": selected_noise_level,
        "selected_noise_output_mode": selected_noise_output_mode,
        "selected_noise_param_jitter": selected_noise_param_jitter,
        "shared_config": {
            "seed": seed,
            "n_samples": n_samples,
            "cl_sample": cl_sample,
            ...
            "atmosphere_backend": "ATMOSPHERE.torch",
            "atmosphere_info": atmosphere_sequence.info,
            "atmosphere_sampled_r0": atmosphere_sequence.sampled_r0,
            "atmosphere_scintillation": atmosphere_scintillation,
            "atmosphere_propagation_mode": atmosphere_propagation_mode,
            "atmosphere_delta_mode": atmosphere_delta_mode,
            "atmosphere_frozen_flow_mode": atmosphere_frozen_flow_mode,
            "atmosphere_subharmonic_mode": atmosphere_subharmonic_mode,
            "loop_sign": loop_sign,
            "use_integrator_clamp": use_integrator_clamp,
            "integrator_limit": integrator_limit,
            "ref_resolution": ref_resolution,
            "ref_diameter": ref_diameter,
            "ref_precision": ref_precision_name,
            "propagation_pupil": {
                "enabled": selected_propagation_enabled,
                "central_obstruction_diam_px": selected_propagation_obstruction_px,
                "spiders": selected_propagation_spiders,
        ...
        "basis_by_model": basis_by_model_info,
        "noise_levels": {},
    }
    ...
    #   noise level -> models
    #
    # The only behavioral change is that each model is loaded immediately before
    # its own test and released immediately afterwards. At no point are all NN/WFS
    # bundles or all model bases resident on the GPU simultaneously.
    # -----------------------------------------------------
    for current_noise_level in noise_levels_to_run:
        current_noise_name = f"noise_{int(current_noise_level):02d}"
        print(f"\nRunning closed-loop test with noise_level={current_noise_level}/10")

        # Keep exactly the original behavior: one noise pipe is created for the
        # current noise level and reused sequentially by every model.
        ...
        output_noise_dir = os.path.join(output_dir, current_noise_name)
        os.makedirs(output_noise_dir, exist_ok=True)

        comparative_wfe = {}
        comparative_strehl = {}
        comparative_wfe_ideal = {}
        comparative_strehl_ideal = {}
        reference_wfe_open = None
        reference_strehl_open = None

        all_results = {}

        for model_index, model_info in enumerate(MODELS_TO_TEST):
            Name = model_info["Name"]
            output_model_test_path = os.path.join(output_noise_dir, Name)

            print(
                f"\nLoading model {model_index + 1}/{len(MODELS_TO_TEST)} "
                f"for noise_level={current_noise_level}: {Name}"
            )

            # Loading/constructing a network can potentially consume random
            # numbers internally. These extra reloads did not exist in the old
            # execution path, so preserve and restore RNG state around the load
            # to avoid perturbing the stochastic camera-noise sequence.
            cpu_rng_state = torch.get_rng_state()
            cuda_rng_states = None
            if torch.cuda.is_available():
                cuda_rng_states = torch.cuda.get_rng_state_all()

            bundle = load_model_bundle(model_info, device=device)
            ...
            # Restore RNG state after loading/building the model-specific runtime
            # objects so the closed-loop stochastic behavior remains isolated from
            # the new sequential loading mechanism.
            ...
            print(
                f"Basis loaded | {Name} | "
                f"basis={model_basis.basis_kind} | n_modes={model_basis.n_modes} | "
                f"path={model_basis.basis_path}"
            )

            # Record the exact same basis metadata that was previously collected
            # in the preliminary validation pass, but only when the model is
            # actually being executed.
            basis_by_model_info[Name] = dict(model_basis.info)

            I_psf_ref_peak = get_model_reference_psf_peak(
            ...
            results = run_closed_loop_single_model(
                ...
                bundle=bundle,
                model_basis=model_basis,
                atmosphere_sequence=atmosphere_sequence,
                output_model_test_path=output_model_test_path,
                I_psf_ref_peak=I_psf_ref_peak,
                noise_pipe=noise_pipe,
                device=device,
            ...
            # Keep the original result collection and comparison behavior intact.
            all_results[Name] = results
            comparative_wfe[Name] = results["wfe_closed"]
            comparative_strehl[Name] = results["strehl_closed"]
            comparative_wfe_ideal[Name] = results["wfe_ideal"]
            comparative_strehl_ideal[Name] = results["strehl_ideal"]

            # Open loop should be identical if all models use the same pupil.
            # If a DM basis uses a slightly different pupil, this is just the
            # first model's open-loop reference in the global comparison plot.
            if reference_wfe_open is None:
                reference_wfe_open = results["wfe_open"]
                reference_strehl_open = results["strehl_open"]

            # -------------------------------------------------
            # CRITICAL MEMORY STEP:
            # Release NN, WFS and model-specific basis before
        ...
        save_comparative_plot_per_model_ideal(
            y_open=reference_wfe_open,
            model_closed_curves_dict=comparative_wfe,
            model_ideal_curves_dict=comparative_wfe_ideal,
            ylabel="WFE RMS [rad]",
            out_path=os.path.join(output_noise_dir, "wfe_compare_all_models.png"),
            cl_sample=cl_sample,
        )

        save_comparative_plot_per_model_ideal(
            y_open=reference_strehl_open,
            model_closed_curves_dict=comparative_strehl,
            model_ideal_curves_dict=comparative_strehl_ideal,
            ylabel="Strehl ratio",
            out_path=os.path.join(output_noise_dir, "strehl_compare_all_models.png"),
            cl_sample=cl_sample,
        )
        ...
        noise_summary = {
            ...
            "shared_config": global_summary["shared_config"],
            "models_to_test": MODELS_TO_TEST,
            "basis_by_model": basis_by_model_info,
            "results_paths": {
                name: os.path.join(output_noise_dir, name, "closed_loop_results.pt")
                for name in all_results.keys()
            },
        }

        torch.save(noise_summary, os.path.join(output_noise_dir, "comparison_summary.pt"))
        global_summary["noise_levels"][int(current_noise_level)] = noise_summary

        print(f"Finished noise_level={current_noise_level}/10")
        print(f"Output folder: {output_noise_dir}")
        print(f" - {os.path.join(output_noise_dir, 'wfe_compare_all_models.png')}")
        print(f" - {os.path.join(output_noise_dir, 'strehl_compare_all_models.png')}")

    torch.save(global_summary, os.path.join(output_dir, "comparison_summary_all_noise_levels.pt"))
